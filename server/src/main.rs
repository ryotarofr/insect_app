...
use insect_app_server::{db, handlers, repos, routes};
...
async fn main() -> anyhow::Result<()> {
    ...
    // TODO(Phase 9): db_pool を AppState に乗せて handler から参照できるようにする。
    //   現状 handler は in-memory store のままなので、pool は warm up + migration 効果のみ。
