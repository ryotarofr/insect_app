...
### 7.7 ValidateA11y trait (Phase 9 前に追加)
...
API handler は **`validate_keys()` の直後に `validate_a11y()` も呼ぶ** ことを規約とする。失敗時は `KeyConflict` と同様 400 で reject し、error body に `template` / `count` を含めて debug 容易にする。

**broken fixture**: `fixtures/cards/broken/multiple_headlines.json` を 1 つ追加し、deserialize は通って `validate_a11y()` で reject されることをテスト。

**将来の拡張**: 同じ trait に「`<button>` の `aria-label` 欠落チェック」「`<img>` の `alt` 欠落チェック」等を順次追加する余地を残す。**`Block::Cta` には現状 `aria-label` フィールドが無い** ため当面は WCAG 上のテキスト要件を `label: Localizable` の存在で担保している (= 別の不変条件として将来コード化対象)。
...
### 13.5 i18n キー網羅 CI (Phase 9 前に追加)

§10.1 で「本番でキー解決失敗時は空文字 + エラーログ」と規定したが、これは**本番環境で文字が消える事故を許容する**設計でもある。CI で **Rust source 中の `I18nKey` 文字列リテラル集合 vs `client_solid/src/i18n/<locale>.json` のキー集合**を突き合わせることで、漏れを deploy 前に検出する。

**Rust 側 extractor**:
- `server/src/sdui/i18n_audit.rs` に `extract_i18n_keys()` を実装。
- 対象: handler / fixture / test 内で `Localizable::I18n { key: "..." }` のように埋まっている文字列リテラル、`I18nKey::new("...")` 呼び出し、`server/fixtures/**/*.json` 内の `{ "source": "i18n", "key": "..." }`。
- 出力: `server/i18n_keys.json` に `["scope.key1", "scope.key2", ...]` のソート済み配列。
- `cargo test` 実行で生成 (= 既存の ts-rs 生成パイプラインと同じトリガ)。

**Node 側 check script**:
- `scripts/check-i18n-keys.mjs` が `server/i18n_keys.json` と `client_solid/src/i18n/*.json` の各 locale を比較。
- 欠落: locale 辞書に存在しない Rust キーがあれば fail (= 本番で空文字事故になる)。
- 余剰: Rust から参照されないキーが locale 辞書にあれば warn (= 死んだ翻訳)。デフォルトでは fail にしないが `--strict` で fail。

**CI 統合**:
- `.github/workflows/ci.yml` (もしくは現行の CI 設定) に `bun run check:i18n` ステップを追加。
- pre-commit hook にも組み込み (= local で気付ける)。

**テスト**: 故意にキーを 1 つ消した locale 辞書で CI が fail することを確認。

**例外**: `validation_error` で server が動的に組み立てるキー (= field 名 × エラー種別) は extractor の対象外とする。これらは別途 enum で網羅する設計とし、§17 Future Work に「validation_error key 網羅 CI」として追記する。

### 13.6 ラウンドトリップ等価性テスト (Phase 9 前に追加)
...
**Rust 側 (`proptest`)**:
- `server/src/sdui/proptest.rs` に `arb_card_block()` を実装。`product_feature` / `product_detail` / `cart` の 3 template それぞれに対し、各 region の Block 配列をランダム生成する戦略を書く。
- `server/tests/sdui_roundtrip.rs` で:
  - `serde_json::to_string(&card) → serde_json::from_str → assert_eq!`
  - `validate_keys()` / `validate_a11y()` を通る生成だけを採用 (= invariant 違反は除外)
  - 1 万件で実行して flaky なら strategy を狭める

**TS 側 (`fast-check`)**:
- `client_solid/src/sdui/__tests__/roundtrip.test.ts` で同等の戦略を `fast-check` で書き、`ajv` で JSON Schema validate。
- branded.ts 経由の型と `generated/sdui.ts` の型の双方で deserialize 可能であることを assert (= branded layer の型レベル差し替えがランタイム互換性を壊していないことを検出)。
...
### 14.5 認証必須エンドポイントの CDN 隔離 (Phase 9 前に追加)
...
**実装方針**:
- edge ロジック (Cloudflare Workers / CloudFront Functions / fastly VCL 等) で上記 prefix に **`Cache-Control: private, no-store` を強制上書き** + `cf-cache-status: BYPASS` のような edge bypass フラグを付与。
