...
`insect_app` の方針転換 (SNS → 業者向け飼育管理) と、それを支える 5 つの中核機能の設計を記す。実装着手前のレビュー資料 + 着手後の整合確認用。価格設定・ヒアリング結果・法務確認など、後日詰める論点は「§ 7. オープン論点」にまとめる。

関連ドキュメント:

- 全体設計概要: `docs/design-overview.md`
- 機能ロードマップ: `docs/features.md` / `docs-site/src/content/docs/planning/features.md`
- DB 設計の正典: `docs/db-schema-design.md` (本書のスキーマ提案は本書クローズ後にこちらへ統合)
...
| アクション | 1 通 (課金対象) |
|---|---|
| 個体 X に対する新規発行 (version 1) | ✅ |
| 個体 X の再発行 (version 2, 3, ...) | ✅ |
| 落札後の所有権移転に伴う新所有者向け発行 | ✅ (買い手側に課金) |
| 発行済み PDF のダウンロード / 印刷 | ❌ (無料、何度でも) |
| 公開ページ (QR 経由) の閲覧 | ❌ |
