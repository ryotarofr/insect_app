...
本ドキュメントは `insect_app` の設計を新規開発者が短時間で把握できるように、アーキテクチャ・データモデル・API/型契約・主要処理フローの 4 観点でまとめたものです。詳細仕様は各実装ファイルおよび `docs-site/` を参照してください。
...
├── docs-site/              # Astro + Starlight ドキュメントサイト
└── docs/                   # 本資料を含む設計ドキュメント
```
...
`/api/v1` 配下に約 40 エンドポイント。`server/src/routes.rs` 参照。
